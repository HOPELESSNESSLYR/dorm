<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="居住天数" prop="livedays">
        <el-input
          v-model="queryParams.livedays"
          placeholder="请输入居住天数"
          clearable
          @keyup.enter.native="handleQuery"
        />
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['feemanage:manday:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['feemanage:manday:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['feemanage:manday:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleImport"
          v-hasPermi="['feemanage:manday:import']"
        >导入</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['feemanage:manday:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="mandayList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="居住天数" align="center" prop="livedays" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="居住天数" prop="livedays">
          <el-input v-model="form.livedays" placeholder="请输入居住天数" />
        </el-form-item>
        <el-form-item label="日期" prop="date">
          <el-date-picker clearable
                          v-model="form.date"
      ...
      >
        ...
        <div class="el-upload__tip text-center" slot="tip">
          <div class="el-upload__tip" slot="tip">
            <el-checkbox v-model="upload.updateSupport" /> 是否更新已经存在的每月入住天数数据
          </div>
...
<script>
import { listManday, getManday, delManday, addManday, updateManday } from "@/api/feemanage/manday";
...
export default {
  ...
  data() {
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + "/feemanage/manday/importData"
      },
    };
  },
  created() {
  ...
  methods: {
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加每月入住天数临时";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const id = row.id || this.ids
      getManday(id).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改每月入住天数临时";
      });
    },
    /** 提交按钮 */
    submitForm() {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除每月入住天数临时编号为"' + ids + '"的数据项？').then(function() {
        return delManday(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('feemanage/manday/export', {
        ...this.queryParams
      }, `manday_${new Date().getTime()}.xlsx`)
    },
    /** 导入按钮操作 */
    handleImport() {
      this.upload.title = "每月入住天数导入";
      this.upload.open = true;
    },
    /** 下载模板操作 */
    importTemplate() {
      this.download('feemanage/manday/importTemplate', {
      }, `每月入住天数信息_模板_${new Date().getTime()}.xlsx`)
    },
    // 文件上传中处理
